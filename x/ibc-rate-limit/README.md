...
* Channel Value: The total value of the chanel denominated in `Denom` (i.e.: channel-17 is worth 10k fury).  
...
For native denoms, we can just use the denom in the packet. If the denom is invalid, it will fail somewhere else along the chain. Example result: `ufury`
...
* The token is a native token, in which case we just remove the prefix added by the counterparty. Example result: `ufury`
...
This means that if we have a daily quota of 1% of the fury supply, and the channel value is 1M fury at the beginning of 
the quota, no more than 100k fury can transferred during that day. If 10M fury were to be minted or IBC'd in during that
