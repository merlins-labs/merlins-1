...
func ValidateBaseDenoms(denoms []BaseDenom) error {
	// The first base denom must be the Merlin denomination
	if len(denoms) == 0 || denoms[0].Denom != MerlinDenomination {
		return fmt.Errorf("the first base denom must be the Merlin denomination")
	}
