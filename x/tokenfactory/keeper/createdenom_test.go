...
func (s *KeeperTestSuite) TestMsgCreateDenom() {
	var (
		...
		denomCreationFee   = sdk.NewCoins(sdk.NewCoin("umer", sdk.NewInt(1000000)))
	)
	...
	res, err = s.msgServer.CreateDenom(sdk.WrapSDKContext(s.Ctx), types.NewMsgCreateDenom("merlin.eth/creator", "bitcoin"))
	s.Require().Error(err)
}
