...
func (s *KeeperTestSuite) TestValidateLockForSFDelegate() {
	...
	tests := []struct {
		...
		},
		{
			...
			lock: &lockuptypes.PeriodLock{
				Owner:    lockOwner.String(),
				Coins:    sdk.NewCoins(sdk.NewCoin("umer", sdk.NewInt(100))),
				Duration: time.Hour * 24 * 21,
				ID:       1,
			},
			superfluidAssetToSet: types.SuperfluidAsset{Denom: DefaultGammAsset, AssetType: types.SuperfluidAssetTypeLPShare},
			expectedErr:          errorsmod.Wrapf(types.ErrNonSuperfluidAsset, "denom: %s", "umer"),
		},
		{
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			for _, lock := range locks {
				...
				merAmount, err := s.App.SuperfluidKeeper.GetSuperfluidMERTokens(s.Ctx, intermediaryAcc.Denom, tc.unlockAmount)
				s.Require().NoError(err)
				...
				s.Require().True(merAmount.IsPositive())
				supplyAfter := s.App.BankKeeper.GetSupply(s.Ctx, bondDenom)
				s.Require().Equal(supplyAfter, supplyBefore.Sub(sdk.NewCoin(bondDenom, merAmount)))
	...
	for _, tc := range testCases {
		...
		s.Run(tc.name, func() {
			...
			for denom, multiplier := range tc.multipliersByDenom {
				s.App.SuperfluidKeeper.SetMerEquivalentMultiplier(s.Ctx, 2, denom, multiplier)
			}
			...
			for interAccIndex, intermediaryAcc := range intermediaryAccs {
				...
				s.Require().NoError(err)
				expAmount := s.App.SuperfluidKeeper.GetRiskAdjustedMerValue(s.Ctx, decAmt.RoundInt())
