...
# fixed point precision used in Merlin `furymath` package.
furymath_precision = 36
...
# truncations to 36 decimal points (the max furymath supported precision).
def exponent_approximation_choice():
    # Equispaced x coordinates to be used for plotting every approximation.
    x_coordinates = approximations.linspace(x_start, x_end, num_points_plot)
    x_coordinates = [sp.N(sp.Float(coef, furymath_precision), n=furymath_precision) for coef in x_coordinates]
    ...
    # coef_numerator = [sp.N(coef, furymath_precision + 2) for coef in coef_numerator]
    # coef_denominator = [sp.N(coef, furymath_precision + 2) for coef in coef_denominator]
    ...
    coef_numerator = [
        sp.N(sp.Float("1.000000000000000000000044212244679434", furymath_precision), n=furymath_precision),
        sp.N(sp.Float("0.352032455817400196452603772766844426", furymath_precision), n=furymath_precision),
        sp.N(sp.Float("0.056507868883666405413116800969512484", furymath_precision), n=furymath_precision),
        sp.N(sp.Float("0.005343900728213034434757419480319916", furymath_precision), n=furymath_precision),
        sp.N(sp.Float("0.000317708814342353603087543715930732", furymath_precision), n=furymath_precision),
        sp.N(sp.Float("0.000011429747507407623028722262874632", furymath_precision), n=furymath_precision),
        sp.N(sp.Float("0.000000198381965651614980168744540366", furymath_precision), n=furymath_precision),
    ]

    coef_denominator = [
        sp.N(sp.Float("1.0000000000000000000000000000000000000", furymath_precision), n=furymath_precision),
        sp.N(sp.Float("-0.341114724742545112949699755780593311", furymath_precision), n=furymath_precision),
        sp.N(sp.Float("0.052724071627342653404436933178482287", furymath_precision), n=furymath_precision),
        sp.N(sp.Float("-0.004760950735524957576233524801866342", furymath_precision), n=furymath_precision),
        sp.N(sp.Float("0.000267168475410566529819971616894193", furymath_precision), n=furymath_precision),
        sp.N(sp.Float("-0.000008923715368802211181557353097439", furymath_precision), n=furymath_precision),
        sp.N(sp.Float("0.000000140277233177373698516010555916", furymath_precision), n=furymath_precision),
    ]
