...
merlin add-genesis-account $(merlin keys show validator -a --keyring-backend=test --home=$HOME/.merlin) 1000000000ufury,1000000000valtoken --home=$HOME/.merlin
sed -i -e "s/stake/ufury/g" $HOME/.merlin/config/genesis.json
merlin gentx validator 500000000ufury --commission-rate="0.0" --keyring-backend=test --home=$HOME/.merlin --chain-id=testing
...
merlin tx gov submit-proposal --title="prop with enough fury deposit" --description="prop w/ enough deposit"  --from=validator --deposit=500000000ufury --chain-id=testing --keyring-backend=test --broadcast-mode=block  --type="Text"
