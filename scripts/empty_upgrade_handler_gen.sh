...
PREV_MERLIN_DEV_TAG=$(curl -L -s 'https://registry.hub.docker.com/v2/repositories/fanfury/merlin-dev/tags?page=1&page_size=100'            | jq -r '.results[] | .name | select(.|test("^(?:v|)[0-9]+\\.[0-9]+(?:$|\\.[0-9]+$)"))' | grep --max-count=1 "")
PREV_MERLIN_E2E_TAG=$(curl -L -s 'https://registry.hub.docker.com/v2/repositories/fanfury/merlin-e2e-init-chain/tags?page=1&page_size=100' | jq -r '.results[] | .name | select(.|test("^(?:v|)[0-9]+\\.[0-9]+(?:$|\\.[0-9]+$)"))' | grep --max-count=1 "")
