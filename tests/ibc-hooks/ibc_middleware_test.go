...
import (
	...
	"github.com/merlinslair/merlin/v16/tests/merlinibctesting"

	"github.com/merlinslair/merlin/v16/tests/ibc-hooks/testutils"
)
...
type HooksTestSuite struct {
	...
	chainC *merlinibctesting.TestChain
...
func (suite *HooksTestSuite) SetupTest() {
	...
	suite.chainC = &merlinibctesting.TestChain{
...
func (suite *HooksTestSuite) fundAccount(chain *merlinibctesting.TestChain, owner sdk.AccAddress) {
...
func (suite *HooksTestSuite) setChainChannelLinks(registryAddr sdk.AccAddress, chainName Chain) {
	...
	msg := `{
		"modify_chain_channel_links": {
		  "operations": [
			{"operation": "set","source_chain": "chainB","destination_chain": "merlin","channel_id": "channel-0"},
			{"operation": "set","source_chain": "merlin","destination_chain": "chainB","channel_id": "channel-0"},
			...
			{"operation": "set","source_chain": "merlin","destination_chain": "chainC","channel_id": "channel-1"},
			{"operation": "set","source_chain": "chainC","destination_chain": "merlin","channel_id": "channel-1"},
			{"operation": "set","source_chain": "merlin","destination_chain": "chainB-cw20","channel_id": "channel-2"},
			{"operation": "set","source_chain": "chainB-cw20","destination_chain": "merlin","channel_id": "channel-2"}
		  ]
		}
	  }
	  `
	...
	msg := `{
		"modify_chain_channel_links": {
		  "operations": [
			...
			{"operation": "change","source_chain": "chainB","destination_chain": "merlin","new_source_chain": "chainD"},
			{"operation": "change","source_chain": "merlin","destination_chain": "chainB","new_destination_chain": "chainD"}
		  ]
		}
	  }
	  `
...
func (suite *HooksTestSuite) TestCrosschainRegistry() {
	...
	contractAlias := "merlin_registry_contract"
	...
	channelQuery := `{"get_channel_from_chain_pair": {"source_chain": "merlin", "destination_chain": "chainB"}}`
	...
	_, err := suite.chainA.GetMerlinApp().WasmKeeper.QuerySmart(suite.chainA.GetContext(), registryAddr, []byte(channelQuery))
	suite.Require().Error(err)
	suite.Require().Contains(err.Error(), "RegistryValue not found")

	// Unwrap token0CB and check that the path has changed
	channelQuery = `{"get_channel_from_chain_pair": {"source_chain": "merlin", "destination_chain": "chainD"}}`
	channelQueryResponse = suite.chainA.QueryContractJson(&suite.Suite, registryAddr, []byte(channelQuery))
	suite.Require().Equal("channel-0", channelQueryResponse.Str)
}
...
func (suite *HooksTestSuite) TestUnwrapToken() {
	...
	testCases := []struct {
		...
		{ChainA, "merlin", token0CBA, token0CA, []Direction{AtoB, BtoC, CtoA}},
	...
	for _, tc := range testCases {
		...
		receiverApp := receiverChain.GetMerlinApp()
		initialSenderBalance := merlinApp.BankKeeper.GetBalance(suite.chainA.GetContext(), owner, tc.sentToken)
		...
		finalSenderBalance := merlinApp.BankKeeper.GetBalance(suite.chainA.GetContext(), owner, tc.sentToken)
...
func (suite *HooksTestSuite) TestCrosschainSwaps() {
	...
	msg := fmt.Sprintf(`{"merlin_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s", "on_failed_delivery": "do_nothing"}}`,
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCTest() {
	...
	// Generate swap instructions for the contract
	swapMsg := fmt.Sprintf(`{"merlin_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s", "on_failed_delivery": "do_nothing", "next_memo":{}}}`,
		receiver,
	)
	// Generate full memo
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCBadAck() {
	...
	swapMsg := fmt.Sprintf(`{"merlin_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s","on_failed_delivery": {"local_recovery_addr": "%s"}, "next_memo": %s }}`,
		receiver,
		recoverAddr,
	...
	swapMsg2 := fmt.Sprintf(`{"merlin_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s","on_failed_delivery": {"local_recovery_addr": "%s"}, "next_memo": %s }}`,
		receiver,
		recoverAddr2,
		`{"wasm": "bad wasm specifier"}`,
	)
	...
	balanceContract2 := merlinAppA.BankKeeper.GetBalance(suite.chainA.GetContext(), crosschainAddr, "token1")
	...
	balanceRecovery := merlinAppA.BankKeeper.GetBalance(suite.chainA.GetContext(), recoverAddr2, "token1")
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCBadSwap() {
	...
	swapMsg := fmt.Sprintf(`{"merlin_swap":{"output_denom":"token1","slippage":{"min_output_amount":"50000"},"receiver":"chainB/%s", "on_failed_delivery": "do_nothing"}}`,
		receiver,
	)
	// Generate full memo
	msg := fmt.Sprintf(`{"wasm": {"contract": "%s", "msg": %s } }`, crosschainAddr, swapMsg)
...
func (suite *HooksTestSuite) TestBadCrosschainSwapsNextMemoMessages() {
	...
	innerMsg := fmt.Sprintf(`{"merlin_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s","on_failed_delivery": {"local_recovery_addr": "%s"},"next_memo":%%s}}`,
...
func (suite *HooksTestSuite) TestCrosschainForwardWithMemo() {
	...
	// Generate swap instructions for the contract
	//
	// Note: Both chains think of themselves as "merlin" and the other as "chainB". That is, the registry
	...
	// be relevant here
	nextMemo := fmt.Sprintf(`{"wasm": {"contract": "%s", "msg": {"merlin_swap":{"output_denom":"token0","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"ibc:channel-0/%s", "on_failed_delivery": "do_nothing"}}}}`,
		crosschainAddrB,
		receiver,
	)
	swapMsg := fmt.Sprintf(`{"merlin_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"ibc:channel-0/%s", "on_failed_delivery": "do_nothing", "next_memo": %s}}`,
		crosschainAddrB,
		nextMemo,
	)
	fmt.Println(swapMsg)
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCMultiHop() {
	...
	merlinAppB := suite.chainB.GetMerlinApp()
	balanceToken0 := merlinAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), accountB, token0ACB)
	balanceToken1 := merlinAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), accountB, token1AB)
	...
	balanceToken0After := merlinAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), accountB, token0ACB)
	suite.Require().Equal(int64(1000), balanceToken0.Amount.Sub(balanceToken0After.Amount).Int64())

	balanceToken1After := merlinAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), accountB, token1AB)
	suite.Require().Greater(balanceToken1After.Amount.Int64(), int64(0))
}
...
func (suite *HooksTestSuite) TestMultiHopXCS() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			sentTokenBalance := senderChain.GetMerlinApp().BankKeeper.GetBalance(senderChain.GetContext(), tc.sender.address, initialToken)
			fmt.Println("sentTokenBalance", sentTokenBalance)
			suite.Require().True(sentTokenBalance.Amount.GTE(sendAmount))
			// get receiver balance
			receivedTokenBalance := receiverChain.GetMerlinApp().BankKeeper.GetBalance(receiverChain.GetContext(), tc.receiver.address, tc.receivedToken)

			// Generate swap instructions for the contract
			swapMsg := fmt.Sprintf(`{"merlin_swap":{"output_denom":"%s","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"%s/%s", "on_failed_delivery": "do_nothing", "next_memo":{}}}`,
			...
			sentTokenAfter := senderChain.GetMerlinApp().BankKeeper.GetBalance(senderChain.GetContext(), tc.sender.address, initialToken)
			suite.Require().Equal(sendAmount.Int64(), sentTokenBalance.Amount.Sub(sentTokenAfter.Amount).Int64())

			fmt.Println(receiverChain.GetMerlinApp().BankKeeper.GetAllBalances(receiverChain.GetContext(), tc.receiver.address))

			receivedTokenAfter := receiverChain.GetMerlinApp().BankKeeper.GetBalance(receiverChain.GetContext(), tc.receiver.address, tc.receivedToken)
...
func (suite *HooksTestSuite) ExecuteOutpostSwap(initializer, receiverAddr sdk.AccAddress, receiver string) {
	...
	// Generate swap instructions for the contract
	swapMsg := fmt.Sprintf(`{"merlin_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"%s", "on_failed_delivery": "do_nothing"}}`,
		receiver,
	)
