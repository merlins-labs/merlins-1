...
make_proposal() {
    ...
    MERLIN_CMD="merlin tx gov submit-proposal software-upgrade \
        $UPGRADE_VERSION \
...
# Query proposal
query_proposal() {
    merlin q gov proposal $PROPOSAL_ID \
        --node $RPC_NODE \
        -o json | jq
}
...
vote_on_proposal() {
    ...
    MERLIN_CMD="merlin tx gov vote $PROPOSAL_ID yes \
        --from $KEY \
        --chain-id $CHAIN_ID \
        --fees $TX_FEES \
        --node $RPC_NODE \
